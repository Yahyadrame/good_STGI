...
export default function ToolsTable() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const data = await response.json();
      setTools([...tools, data]);
      setNewTool({ name: '', description: '', type: '', location: '', image: '' });
      setShowForm(false);
    } catch (error) {
        ...
        <form onSubmit={handleSubmit} className="bg-white p-4 rounded-lg shadow mb-4">
          ...
          <div className="grid gap-4">
            <div>
              ...
              <Input id="name" name="name" value={newTool.name} onChange={handleChange} required />
            </div>
            <div>
              <Label htmlFor="description">Description</Label>
              <Textarea id="description" name="description" value={newTool.description} onChange={handleChange} />
            </div>
            <div>
              <Label htmlFor="type">Type</Label>
              <Input id="type" name="type" value={newTool.type} onChange={handleChange} />
            </div>
            <div>
              <Label htmlFor="location">Lieu</Label>
              <Input id="location" name="location" value={newTool.location} onChange={handleChange} />
            </div>
            <div>
              <Label htmlFor="image">Image (URL)</Label>
              <Input id="image" name="image" value={newTool.image} onChange={handleChange} />
            </div>
            <div className="flex space-x-2">
              <Button type="submit" className="bg-blue-600 hover:bg-blue-700 text-white">Enregistrer</Button>
              <Button type="button" onClick={() => setShowForm(false)} className="bg-gray-600 hover:bg-gray-700 text-white">
                Annuler
              </Button>
            </div>
          </div>
